...
/* addr_deserialize */
static na_return_t
na_ofi_addr_update(na_class_t *na_class, na_addr_t *addr, unsigned long flags);
...
NA_PLUGIN const struct na_class_ops NA_PLUGIN_OPS(ofi) = {
    ...
    na_ofi_addr_update,                    /* addr_update */
    ...
    fi_addr_t src_addr, struct na_ofi_addr **na_ofi_addr_p)
{
    ...
    if (na_ofi_class->domain->av_user_id) {
        na_ofi_addr = (struct na_ofi_addr *) src_addr;
    } else {
        na_ofi_addr = na_ofi_fi_addr_map_lookup(&na_ofi_class->domain->addr_map, &src_addr);
        if (na_ofi_addr == NULL) {
                struct na_ofi_addr_key  addr_key;
                size_t addrlen = sizeof(addr_key);
                fi_av_lookup(na_ofi_class->domain->fi_av, src_addr, &addr_key.addr, &addrlen);
                na_ofi_addr_key_lookup(na_ofi_class, &addr_key, src_addr, &na_ofi_addr);
        }
    }
...
static na_return_t
na_ofi_addr_update(na_class_t *na_class, na_addr_t *addr,
